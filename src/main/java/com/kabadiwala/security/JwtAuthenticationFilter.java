...
public class JwtAuthenticationFilter extends org.springframework.web.filter.OncePerRequestFilter {
    ...
    // ✅ Whitelisted endpoints that should bypass JWT check
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        logger.info("This is in JwtFilter");
    ...
    private boolean isWhitelisted(String path) {
        return Arrays.stream(WHITELIST).anyMatch(whitelist -> {
            if (whitelist.endsWith("/**")) {
                return path.startsWith(whitelist.substring(0, whitelist.length() - 3));
            } else if (whitelist.endsWith("**")) {
                return path.startsWith(whitelist.substring(0, whitelist.length() - 2));
            } else {
                return path.equals(whitelist) || path.startsWith(whitelist + "/");
            }
        });
    }
